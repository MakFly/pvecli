# PRD — `pvectl` : CLI d'administration Proxmox VE
...
> **`pvectl` est une CLI Go qui pilote un homelab Proxmox de bout en bout — de l'inspection au CRUD complet — et qui sert de pont entre l'API PVE et la chaîne Terraform / Ansible.**
...
| Compétence | Preuve attendue |
| --- | --- |
| Frontière CLI ↔ Terraform | Savoir dire *qui possède quoi* : ce que `pvectl` crée à la main vs ce que Terraform possède dans son state |
| Détection de dérive | `pvectl iac drift` compare l'état live (API) au state Terraform et liste les écarts |
| Inventaire dynamique | `pvectl iac inventory` génère un inventaire Ansible depuis l'API (tags, agent QEMU, IP réelles) |
| Idempotence | Rejouer une commande `apply` deux fois ne produit aucun changement |
| Chaînage complet | `template → clone (TF) → inventaire (pvectl) → configuration (Ansible) → validation (pvectl)` |

### 3.3 Non-objectifs (v1)

- Pas de TUI interactive (une `pvectl top` pourra arriver plus tard).
...
- Pas de réimplémentation de Terraform : `pvectl` *observe* et *alimente* l'IaC, il ne la remplace pas.
   ...
   └────────────────────┼───┘          │
                        ...
                        └────►│  inventory.yml       │
                              ...
                              └─────┬────────────────┘
                                    │
                              ┌─────▼────────────────┐
                              │ pvectl vm exec-check │
                              │ (validation finale)  │
                              └──────────────────────┘
```

**Contrat de propriété** : une ressource portant le tag `managed` (convention du repo : `tags = ["lab","terraform","managed"]`) est **interdite en écriture** à `pvectl`, sauf `--force-unmanaged`. `pvectl` refusera `vm rm 210` si la VM est taguée `managed` — c'est Terraform qui doit la détruire. Cette règle est le cœur de l'apprentissage IaC.
...
```
pvectl <domaine> <verbe> [cible] [flags]
```
...
| --- | --- | --- |
| `pvectl version` | `GET /version` | R |
| `pvectl cluster status` | `GET /cluster/status` | R |
| `pvectl cluster resources` | `GET /cluster/resources` | R |
| `pvectl node ls` | `GET /nodes` | R |
| `pvectl node show <node>` | `GET /nodes/{n}/status` | R |
| `pvectl vm ls [--node] [--tag]` | `GET /nodes/{n}/qemu` | R |
| `pvectl vm show <vmid>` | `GET .../qemu/{id}/config` + `/status/current` | R |
| `pvectl vm create` | `POST /nodes/{n}/qemu` | **W** |
| `pvectl vm clone <src> --newid` | `POST .../qemu/{id}/clone` | **W** |
| `pvectl vm set <vmid> --cores --memory …` | `PUT .../qemu/{id}/config` | **W** |
| `pvectl vm start\|stop\|shutdown\|reboot\|reset <vmid>` | `POST .../qemu/{id}/status/{action}` | **W** |
| `pvectl vm migrate <vmid> --target` | `POST .../qemu/{id}/migrate` | **W** |
| `pvectl vm rm <vmid>` | `DELETE .../qemu/{id}` | **W‼** |
| `pvectl vm snapshot ls\|create\|rollback\|rm` | `GET/POST .../qemu/{id}/snapshot[...]` | R/**W** |
| `pvectl vm template <vmid>` | `POST .../qemu/{id}/template` | **W** |
| `pvectl vm agent <vmid> ifaces` | `GET .../qemu/{id}/agent/network-get-interfaces` | R |
| `pvectl lxc …` | idem sur `/nodes/{n}/lxc` | R/**W** |
| `pvectl storage ls` | `GET /storage`, `GET /nodes/{n}/storage` | R |
| `pvectl storage content <store>` | `GET /nodes/{n}/storage/{s}/content` | R |
| `pvectl storage download-url <store>` | `POST .../storage/{s}/download-url` | **W** |
| `pvectl backup run <vmid>` | `POST /nodes/{n}/vzdump` | **W** |
| `pvectl backup ls` | `GET /nodes/{n}/storage/{s}/content?content=backup` | R |
| `pvectl backup restore <archive> --newid` | `POST /nodes/{n}/qemu` (param `archive`) | **W‼** |
| `pvectl task ls [--running]` | `GET /nodes/{n}/tasks` | R |
| `pvectl task show <upid>` | `GET /nodes/{n}/tasks/{upid}/status` | R |
| `pvectl task log <upid>` | `GET /nodes/{n}/tasks/{upid}/log` | R |
| `pvectl task wait <upid>` | polling jusqu'à état terminal | R |
| `pvectl access user ls` | `GET /access/users` | R |
| `pvectl access token create <user> <id>` | `POST /access/users/{u}/token/{t}` | **W‼** |
| `pvectl access acl ls` | `GET /access/acl` | R |
| `pvectl access acl set --path --role --token` | `PUT /access/acl` | **W‼** |
| `pvectl access whoami` | `GET /access/permissions` | R |
| `pvectl net ls <node>` | `GET /nodes/{n}/network` | R |
| `pvectl net apply <node>` | `PUT /nodes/{n}/network` | **W‼** |
| `pvectl pool ls\|create\|rm` | `/pools` | R/**W** |
| `pvectl iac inventory [--tag] [-o file]` | lecture API → YAML Ansible | R |
| `pvectl iac drift [--tfstate]` | API ∩ state Terraform | R |
| `pvectl iac adopt <vmid>` | génère un bloc `import` Terraform | R |
| `pvectl iac plan\|apply` | wrapper `terraform` + pré/post-checks API | **W** |
| `pvectl iac configure [--limit]` | wrapper `ansible-playbook` avec inventaire généré | **W** |
| `pvectl config init\|show\|set` | fichier local | — |
| `pvectl completion bash\|zsh\|fish` | Cobra | — |
...
Discipline stdout/stderr : **les données vont sur stdout, tout le reste (progression, avertissements, confirmations) sur stderr.** `pvectl vm ls -o json | jq` doit toujours fonctionner.
...
| 403 | `privilège manquant sur <path> — vérifier ACL, propagation, privilege separation du token (pvectl access whoami)` |
...
| lock | `ressource verrouillée — pvectl task ls --running` |
...
| --- | --- | --- | --- |
| **M0** Socle | 00, 01 | `main.go`, Cobra, config layering, client HTTP, TLS pinning, `version`, `node ls` | `pvectl version` renvoie la version réelle du nœud, en TLS vérifié, avec un token non-root |
| **M1** Lecture | 02 | `vm ls`, `lxc ls`, `storage ls/content`, `task ls/log`, renderers table/json | `pvectl vm ls -o json \| jq '.[].name'` fonctionne ; `docs/API-MAP.md` référence chaque endpoint avec sa source |
...
| **M3** Cycle de vie | 04, 05 | `vm create`, `clone`, `set`, `template`, `snapshot *`, `agent ifaces`, `--dry-run` | Un template cloud-init est créé puis cloné intégralement par `pvectl`, sans passer par l'UI |
...
| **M5** Backup & PRA | 07 | `backup run/ls/restore`, suivi de tâche longue | Une VM est détruite puis restaurée par `pvectl`, RPO/RTO mesurés et notés |
...
| **M7** Finition | 09, 10 | `net`, `pool`, complétion shell, `Makefile`, release cross-platform | Binaire installé et utilisable depuis le nœud lui-même ; `pvectl completion zsh` opérationnelle |
...
- Binaire statique `pvectl`, cross-compilé (`darwin/arm64` pour le poste, `linux/amd64` pour le nœud) via `go build` / GoReleaser en v1.1.
- Version injectée au build (`-ldflags "-X main.version=..."`), affichée par `pvectl --version` (à distinguer de `pvectl version` = version du nœud PVE).
...
1. `pvectl` provisionne une VM depuis un template cloud-init, la configure via Ansible et sert une page Nginx.
2. `pvectl iac drift` détecte une modification faite manuellement dans l'UI.
3. Une VM détruite est restaurée depuis une sauvegarde, avec preuve par relecture API.
4. Le token utilisé n'a **que** les privilèges nécessaires, et `pvectl access whoami` le prouve.
...
pveum user token add automation@pve pvectl --privsep 1 --expire <timestamp>
...
Implémentée comme `pvectl doctor` : vérifie endpoint, TLS, token, privilèges, et affiche un diagnostic actionnable.
