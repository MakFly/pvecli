...
> **Preuve de fin de lot** : `pvectl version` renvoie la version réelle du nœud `192.0.2.23`, en TLS vérifié (pas `--insecure`), avec un token non-root.
...
- Étant donné un dossier vide, quand je lance `go build ./...`, alors un binaire `pvectl` est produit.
- `pvectl` sans argument affiche l'aide et sort en code `0`.
- `pvectl --version` affiche la version du binaire, injectée au build via `-ldflags "-X main.version=..."`.
- L'arborescence `cmd/`, `internal/{config,pve,service,output,log}`, `testdata/`, `docs/` existe.
- Un `Makefile` expose `build`, `test`, `lint`, `fmt`.

**Preuve**
```bash
make build && ./pvectl --version   # → pvectl dev (commit abc1234)
```

**Ce que ça doit t'apprendre** — La distinction `pvectl --version` (version du binaire) vs `pvectl version` (version du nœud PVE) : deux notions que la plupart des CLI confondent.
...
**Critères d'acceptation**
- Étant donné `~/.config/pvectl/config.yaml` contenant un contexte `lab`, quand je lance une commande sans flag, alors l'endpoint du contexte courant est utilisé.
...
- `pvectl config init` crée le fichier avec les bons droits (`0600`), `config show` l'affiche **avec le secret masqué**, `config set <clé> <valeur>` le modifie.
...
**Preuve**
```bash
pvectl config init && pvectl config show   # secret absent, endpoint=https://192.0.2.23:8006
PVE_API_URL=https://autre:8006 pvectl config show | grep autre
```
...
- Une sous-commande `pvectl config trust` récupère l'empreinte du serveur, l'affiche et demande confirmation avant de l'enregistrer.

**Preuve**
```bash
pvectl config trust                 # affiche l'empreinte de 192.0.2.23, demande confirmation
pvectl version                      # fonctionne SANS --insecure
pvectl version --insecure 2>&1 >/dev/null | grep -i warning
```
...
### PVX-005 — Afficher la version du nœud (`pvectl version`)
...
**Preuve**
```bash
pvectl version    # → PVE 8.x.y (release …, repoid …)
```
...
En tant qu'opérateur, je veux `pvectl node ls` et `node show <node>`, afin de connaître le nom exact du nœud que toutes les autres commandes exigeront dans leur chemin.
...
**Preuve**
```bash
pvectl node ls    # → une ligne, le nœud du lab, statut online
```
...
- Mapping conforme au tableau du PRD §7.5 : `401` (format d'en-tête, token_id, secret, realm, expiration), `403` (ACL, propagation, privilege separation → suggère `pvectl access whoami`), `400` (schéma → suggère `search-pve-api.ts`), `404` (node/vmid/storage/version), verrou, échec de tâche.
...
**Preuve**
```bash
PVE_API_TOKEN_SECRET=faux pvectl node ls; echo $?   # → 3, message parlant du token
```
...
### PVX-008 — Diagnostiquer la cible (`pvectl doctor`)
...
pvectl node ls -vv 2>&1 | grep -c "PVEAPIToken=" # → 0
```
